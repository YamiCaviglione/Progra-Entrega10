...
```typescript
{
  ...
  name?: string,
  favorites: ObjectId[], // Referencias a libros favoritos
  createdAt: Date
}
```
...
```typescript
{
  userId: ObjectId, // Referencia al usuario autor
  bookId: string,   // ID del libro de Google Books API
  rating: number,   // 1-5 estrellas
  text?: string,    // Texto opcional de la reseña
  upvotes: number,  // Contador de votos positivos
  downvotes: number, // Contador de votos negativos
  createdAt: Date
}
```

**3. Votaciones (Vote.ts)**
```typescript
{
  userId: ObjectId,   // Usuario que vota
  reviewId: ObjectId, // Reseña votada
  vote: number        // -1 (downvote) o +1 (upvote)
}
```
...
**Tests del Ejercicio 9 Mantenidos (14 tests):**
- `BookCard.test.tsx` - Renderizado de componentes
- `Layout.test.tsx` - Layout con header/footer
- `BookPage.test.tsx` - Integración de página de libro
- `googleBooks.test.ts` - API de Google Books
- `reviews.test.ts` - Operaciones CRUD de reseñas

**Tests de Autenticación Nuevos (6 tests):**
```typescript
describe('requireAuth middleware', () => {
  ...
  it('debe rechazar cuando usuario no existe');
  it('debe manejar error cuando JWT_SECRET no está definido');
  it('debe manejar error de base de datos');
});
```

**Tests de Validación Nuevos (7 tests):**
```typescript
describe('validateBody middleware', () => {
  it('debe validar datos válidos de registro');
  it('debe rechazar email inválido');
  it('debe rechazar contraseña muy corta');
  it('debe validar datos válidos de login');
  it('debe rechazar campos faltantes en login');
  it('debe validar datos válidos de review');
  it('debe rechazar rating fuera de rango');
});
```
## Conclusión
